<template>
    <div class="attrs" style="flex-wrap: nowrap;min-height: 100%">
        <div class="item w-100" style=";padding: 20px">
            <div class="order-detail__item flex-wrap justify-content-between"
                ...
                <div class="d-flex flex-wrap">
                    <div role="button" class="item-processing me-3" @click="page=0" :class="{active:page===0}">
                        <ic_list></ic_list>
                        <div class="me-2">آدرس سایت</div>
                    </div>
                    <div role="button" class="position-relative item-processing me-3" @click="page=1" :class="{active:page===1}">
                        <ic_document_tick> </ic_document_tick>
                        <div class="me-2">نظر مشتری</div>
                    </div>
                ...
                <div v-if="isRun" class="item-processing me-3">
                    ...
                    <div class="me-2">{{prop.attrs.finish[0].created_at_fa}}</div>
                </div>
            </div>
            <div v-if="page===0">
            ...
            <div v-else-if="attrs.finish" class="order-detail mt-4  position-relative">
                <span class="data-left" style="right: unset; left: 30px; top: 20px;">
                    {{prop.attrs.finish[0].updated_at_fa}}
                </span>
                {{attrs.finish [0].name}}
            </div>
        </div>
...
const isRun = ref(!!prop.attrs.finish)
const page = ref(0)
const form = useForm({
    url:prop.attrs.finish?prop.attrs.finish[0].value:null,
    info:prop.attrs.finish?prop.attrs.finish[0].description:null,
